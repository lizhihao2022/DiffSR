...
class SpectralUpsampleConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2, upsample_factor=[2, 2]):
        super(SpectralUpsampleConv2d, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        # Number of Fourier modes to multiply, at most floor(N/2) + 1
        self.modes1 = modes1
    ...
    def forward(self, x):
        ...
        # Multiply relevant Fourier modes
        out_ft = torch.zeros(batchsize, self.out_channels, x.size(-2) * self.upsample_factor[0], x.size(-1) // 2 * self.upsample_factor[1] + 1, device=x.device, dtype=torch.cfloat)
        out_ft[:, :, :self.modes1, :self.modes2] = \
            compl_mul2d(x_ft[:, :, :self.modes1, :self.modes2], self.weights1)
        out_ft[:, :, -self.modes1:, :self.modes2] = \
            compl_mul2d(x_ft[:, :, -self.modes1:, :self.modes2], self.weights2)
        ...
        # Return to physical space
        x = torch.fft.irfftn(out_ft, s=(x.size(-2)*self.upsample_factor[0], x.size(-1)*self.upsample_factor[1]), dim=[2, 3])
        return x
